...
function setupTimer(item_to_link) {
      $j(item_to_link).everyTime(1000, 'importer', function() {
          
            //if ($j("#importer-status-msg").text().strip() != "Opening File...") {
                loadImportProgress();
            //}
            ...
            {
                $j(item_to_link).stopTime('importer');
            ...
            success: function (data)
            {
                ...
                else
                {
                    $j('div#import-action-div').block({ 
                        message: '<h2><img src="/images/interface/busy.gif" /> Just a moment...</h2>', 
                        css: {
